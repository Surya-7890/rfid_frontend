...
const getTotalEntry = async()=>{
    const res = await axios.get('https://rfidbackendsece.cyclic.app/gettotal/entry');
    const data = await res.data;
    return data;
}

const getTotalExit = async()=>{
    const res = await axios.get('https://rfidbackendsece.cyclic.app/gettotal/exit');
    const data = await res.data;
    return data;
}

const fetchIntermediates = async()=>{
    const res = await axios.get('https://rfidbackendsece.cyclic.app/getintermediates');
    const data = await res.data;
    return data;
}
...
export default function Home(){
    ...
    const Intermediates = useQuery({
        queryKey: ['Intermediates'],
        queryFn: fetchIntermediates,
        refetchInterval: 1000,
        refetchIntervalInBackground: true
    });

    if(Entry.isLoading || Exit.isLoading || Intermediates.isLoading){
        return (
            <>
            <div className='bg-ylw'>
            <div className='bg-slate-200 bg-opacity-30 absolute top-0 flex w-screen justify-center h-10 items-center text-xl'><div className='absolute right-[460px] font-bold'>MAKERSPACE DASHBOARD</div></div>
            <div className='relative'>
                ...
                <div className='absolute top-64 left-96 flex justify-around w-4/6 ml-8 h-max'>
                    ...
                    <div>
                    <div className='h-56 w-56 bg-blue-500 rounded-full absolute flex justify-center items-center'><div className='h-44 w-44 bg-ylw rounded-full font-semibold text-3xl flex justify-center pt-5'></div></div>
                   ...
                   <div className='relative text-white top-28 left-[72px] bg-blue-500 w-fit px-5 py-2 mt-2'>Count</div>
                    ...
                    <div className='ml-8'>
                    <div className='h-56 w-56 bg-green-500 rounded-full absolute flex justify-center items-center'><div className='h-44 w-44 bg-ylw rounded-full font-semibold text-3xl flex justify-center pt-5'></div></div>
                   ...
                   <div className='relative text-white top-28 left-[60px] bg-green-500 w-fit px-5 py-2 mt-2'> IN Count</div>
                    ...
                    <div className='h-56 w-56 bg-red-500 rounded-full absolute flex justify-center items-center'><div className='h-44 w-44 bg-ylw rounded-full font-semibold text-3xl flex justify-center pt-5'></div></div>
    ...
    if(Intermediates.data.length === 1) {
        console.log(Intermediates.data.length);
        setTimeout(()=>{
